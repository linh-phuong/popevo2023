...
with col1:
    st.plotly_chart(make_interactive_video(0.01, zlist[-1], 0.03, zlist, invasion_fitness, alpha))


with col2:
    zm = st.slider("Mutant trait value", 0.0, 2.0, value=0.2, step=0.01)
    st.plotly_chart(plot_invasionfitness(zm, zlist, invasion_fitness, alpha))
...
with col5:
    st.plotly_chart(
        make_interactive_video(0.1, zlist[-1], 0.03, zlist, invasion_fitness2, (alpha, beta))
    )
with col6:
    zm2 = st.slider("Mutant trait value  ", 0.0, 2.0, value=0.2, step=0.01)
    st.plotly_chart(plot_invasionfitness(zm2, zlist, invasion_fitness2, (alpha, beta)))
