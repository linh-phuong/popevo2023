...
R = st.sidebar.number_input("R", 1e-3, 0.2, value=0.05, step=1e-4, format="%.4f")
st.sidebar.write(r"The two models give similar results when $R = e^r - 1 = $", np.exp(r) - 1)
...
df = pd.DataFrame(
    dict(time=t_discrete, pop_discrete=pop_discrete, pop_continuous=pop_continuous[:, 0])
)
